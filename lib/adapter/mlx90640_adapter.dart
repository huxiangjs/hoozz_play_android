...
abstract class MLX90640Adapter {
  ...
  static const EventChannel _eventChannel =
      EventChannel('MLX90640_EVENT', StandardMethodCodec());
    ...
    _subscription = stream.listen(
      (event) {
        if (event is String) {
          ...
          Int32List intList =
              Int32List.fromList(colors.map((color) => color.value).toList());
          Uint8List uint8List = intList.buffer.asUint8List();
          Bitmap bitmap = Bitmap.fromHeadless(32, 24, uint8List);

          // Use the then method to register a callback function
          bitmap.buildImage().then((image) {
            ...
            onChanged(TYPE_UPDATE_TEMP_DATA,
                [min, max, data[data.length ~/ 2 + 16], minOffset, maxOffset]);
  ...
  static void setTaShift(
      double newValue, void Function(bool success, double value) result) {
    _methodChannel
        .invokeMethod('setTaShift', {"value": newValue}).then((value) {
      if (value == 0) {
        taShift = newValue;
        result(true, taShift);
      } else {
  ...
  static void setEmissivity(
      double newValue, void Function(bool success, double value) result) {
    _methodChannel
        .invokeMethod('setEmissivity', {"value": newValue}).then((value) {
      if (value == 0) {
        emissivity = newValue;
  ...
  static void setRefreshRate(
      int newValue, void Function(bool success, int value) result) {
    _methodChannel
        .invokeMethod('setRefreshRate', {"value": newValue}).then((value) {
      if (value == 0) {
